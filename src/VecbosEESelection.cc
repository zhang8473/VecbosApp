...
VecbosEESelection::VecbosEESelection(TTree *tree) 
  : Vecbos(tree) {
...
VecbosEESelection::~VecbosEESelection(){
...
void VecbosEESelection::Loop() {
  ...
  for (Long64_t jentry=0; jentry<maxEvents;jentry++) {
	...
	for (int iMc=0; iMc<nMc; iMc++) { 
      ...
      if (_commonSel->passCut("ptHardElectronAcc",thisPtTmp)) {
        ...
        for(int ii=0;ii<howManyRecoEle;ii++) {             
          ...
          TVector3 pLepton(pxEle[ii],pyEle[ii],pzEle[ii]);
          float thisPt=pLepton.Pt();
          if (!_commonSel->passCut("etaElectronAcc",etaEle[ii]) || !anaUtils.isInECALFiducial(fiducialFlagsEle[ii]) )  isGoodEle = false;
	  if (!_commonSel->passCut("ptSlowElectronAcc",thisPt) ) isGoodEle = false;
          ...
          if(anaUtils.fiducialFlagECAL(fiducialFlagsEle[ii],isEB)) 
	    {
	      combinedIsolation.push_back(  (dr03TkSumPtEle[ii] + (float) TMath::Max(float(dr03EcalRecHitSumEtEle[ii]-1.),float(0.)) + dr03HcalTowerSumEtEle[ii]) / pLepton.Pt() );
	    }
	  else
	    {
	      combinedIsolation.push_back(  (dr03TkSumPtEle[ii] + dr03EcalRecHitSumEtEle[ii] + dr03HcalTowerSumEtEle[ii]) / pLepton.Pt() );
	    }
        }
      }
      ...
      for(int ii=0;ii<howManyAccEle;ii++) {      
        ...
        isEleID(&EgammaTightID_NC,acceptElectrons[ii],&eleId,&isol,&conv);
        if (_commonSel->getSwitch("eleId") && !eleId) isGoodEle = false;
      ...
      if (_commonSel->getSwitch("asymmetricElectrons")) {   
        for(int ii=0;ii<howManyAccEle;ii++) {      
          ...
          bool eleId, isol, conv;
          eleId = isol = conv = false;
          isEleID(&EgammaLooseID_NC,acceptElectrons[ii],&eleId,&isol,&conv);
          if ((_commonSel->getSwitch("eleId") && eleId) || 
      ...
      for(int ii=0;ii<howManyIdTightEle;ii++) {      
        ...
        if( _commonSel->getSwitch("isolation") && !isol ) isGoodEle = false;
      ...
      if (_commonSel->getSwitch("asymmetricElectrons")) {   
        for(int ii=0;ii<howManyIdLooseEle;ii++) {      
          ...
          bool eleId, isol, conv;
          eleId = isol = conv = false;
          isEleID(&EgammaLooseID_NC,idEleIndex,&eleId,&isol,&conv);
      ...
      for(int ii=0;ii<howManyIsolTightEles;ii++) {      
        ...
        if( _commonSel->getSwitch("conversionRejection") && !conv ) isGoodEle = false;
        if (isGoodEle) convRejTightElectrons.push_back(isolEleIndex); 
      }
      int howManyConvRejTightEles = convRejTightElectrons.size();
    
      // loose optimized, loose conversion rejection electrons if needed
      int howManyConvRejLooseEles = 0;
      if (_commonSel->getSwitch("asymmetricElectrons")) {   
        for(int ii=0;ii<howManyIsolLooseEles;ii++) {      
          bool isGoodEle = true;
          int isolEleIndex=isolLooseElectrons[ii];
          
          bool eleId, isol, conv;
          eleId = isol = conv = false;
          isEleID(&EgammaLooseID_NC,isolEleIndex,&eleId,&isol,&conv);
          
          if( _commonSel->getSwitch("conversionRejection") && !conv ) isGoodEle = false;
          if (isGoodEle) convRejLooseElectrons.push_back(isolEleIndex); 
        }
        howManyConvRejLooseEles = convRejLooseElectrons.size();
      }
      ...
      int theConvRejEle1_GSFTrack = gsfTrackIndexEle[theConvRejEle1];

      int howManyDzVertexEles = 0; 
      int howManyDzVertexLooseEles = 0; 
      int howManyDzVertexTightEles = 0; 
      float EWKz = trackVzGsfTrack[theConvRejEle1_GSFTrack];
      ...
      if(nPV>0) {         // a primary vertex was found
        ...
        for(int eleList=0; eleList<2; eleList++) {
          ...
          int nConvRejEles = (eleList==0) ? howManyConvRejTightEles : howManyConvRejLooseEles;

          // electron consistency with primary vertex
          for(int iconv=0; iconv<nConvRejEles; iconv++) {
            bool isGoodEle = true;

            int iele = (eleList==0) ? convRejTightElectrons[iconv] : convRejLooseElectrons[iconv];
	...
	// di-jet system pT  - chiara, nel tree
        diPFJetPtW[0] = GetDiJetPt(goodWPFJets_[0]);
        diPFJetPtW[1] = GetDiJetPt(goodWPFJets_[1]);

	// deltaEta between the two jet - chiara, nel tree
        deltaEtaPFJetW[0] = GetDeltaEtaJet(goodWPFJets_[0]);
        deltaEtaPFJetW[1] = GetDeltaEtaJet(goodWPFJets_[1]);

	// deltaPhi between leading jet and MET - chiara, nel tree
	deltaPhiJetMetW[0] = GetDeltaPhiJetMet(goodWPFJets_[0], p3Met_);
	deltaPhiJetMetW[1] = GetDeltaPhiJetMet(goodWPFJets_[1], p3Met_);
	...
	// di-jet system pT  - chiara, nel tree
        diPFJetPtZ[0] = GetDiJetPt(goodZPFJets_[0]);
        diPFJetPtZ[1] = GetDiJetPt(goodZPFJets_[1]);

	// deltaEta between the two jet - chiara, nel tree
        deltaEtaPFJetZ[0] = GetDeltaEtaJet(goodZPFJets_[0]);
        deltaEtaPFJetZ[1] = GetDeltaEtaJet(goodZPFJets_[1]);

	// deltaPhi between leading jet and MET - chiara, nel tree
	deltaPhiJetMetZ[0] = GetDeltaPhiJetMet(goodZPFJets_[0], p3Met_);
	deltaPhiJetMetZ[1] = GetDeltaPhiJetMet(goodZPFJets_[1], p3Met_);
	...
	// to apply the majority method for charge asymmetry
	bool chargeMajMethod=false;
	if(theEle1_>-1) {
	  if(trackIndexEle[theEle1_]>=0){
	    if ((chargeTrack[trackIndexEle[theEle1_]] == scPixChargeEle[theEle1_])&& (chargeTrack[trackIndexEle[theEle1_]] == chargeGsfTrack[gsfTrackIndexEle[theEle1_]])) chargeMajMethod=true;
	  }
	}
        ...
        commonData.nConvRejLooseEle = howManyConvRejLooseEles;
        commonData.nPV = nPV;
	...
	dataWenuPFJet.chargeMajorityMethod = chargeMajMethod;
      ...
      if (isWenuUpToEnd[0] || isWenuUpToEnd[1] || isWenuUpToEndPFJet[0] || isWenuUpToEndPFJet[1]) {       // filling reduced trees for the W->enu study
        ...
        myOutTree_Wenu -> fillMoreElectrons(esc_, eseed_, pin_);
        myOutTree_Wenu -> fillKinematics( -1., WCalomT_, WTCmT_, WPFmT_, GetPt(pxMet[0],pyMet[0]), GetPt(pxTCMet[0],pyTCMet[0]), GetPt(pxPFMet[0],pyPFMet[0]), p3W_.Perp(), U_.Mag(), -1., 
					  diPFJetMassW, diPFJetPtW, deltaEtaPFJetW,deltaPhiJetMetW,
                                          p3W_.Eta(), -1., -1., -1.);
        ...
        float deltaMin  = 999.;
        if (theEle1_>-1) {
          TVector3 pElectron(pxEle[theEle1_],pyEle[theEle1_],pzEle[theEle1_]);
          for (int thePFEle=0; thePFEle<nPFEle; thePFEle++) {
            TVector3 pPFelectron(pxPFEle[thePFEle],pyPFEle[thePFEle],pzPFEle[thePFEle]);
            float thisDeltaR = pElectron.DeltaR(pPFelectron);
            if (fabs(thisDeltaR)<deltaMin) {
              deltaMin    = thisDeltaR;
        ...
        if (theSelPFele>=0) {
          ...
          myOutTree_Wenu -> fillPFelectrons(selPFelectron.Pt(), selPFelectron.Eta(), selPFelectron.Phi(), deltaMin, MvaOutputPFEle[theSelPFele],nPFEle);
        }
        if (theSelPFele<0 && theEle1_>-1)
          myOutTree_Wenu -> fillPFelectrons(-999., -999., -999., -999., -999.,nPFEle);
        if (theSelPFele<0 && theEle1_==-1)
          myOutTree_Wenu -> fillPFelectrons(-1., -1., -1., -1., -1.,nPFEle);
	...
	NWPFjetMatrix15->Fill(numeroPFmc1, numeroGEN1);
      ...
      if (isZeeUpToEnd[0] || isZeeUpToEnd[1] || isZeeUpToEndPFJet[0] || isZeeUpToEndPFJet[1] ) {        // filling reduced trees for the Z->ee study
        ...
        myOutTree_Zee -> fillElectrons(recoflag_, pt_, eta_, phi_,
                                       classification_, nbrems_, deta_, dphi_, hoe_, see_, spp_, eop_, fbrem_,
                                       trackerIso_, hcalIso_, ecalJIso_, ecalGTIso_, combinedIso_, charge_, missHits_, dist_, dcot_, lh_, e9esc_,e25esc_);
        myOutTree_Zee -> fillMoreElectrons(esc_, eseed_, pin_);
        myOutTree_Zee -> fillKinematics( mee_, -1., -1., -1., GetPt(pxMet[0],pyMet[0]), GetPt(pxTCMet[0],pyTCMet[0]), GetPt(pxPFMet[0],pyPFMet[0]), -1., -1., pt3Z_.Mag(), 
					 diPFJetMassZ, diPFJetPtZ,deltaEtaPFJetZ,deltaPhiJetMetZ,
                                         -1., p3Z_.Eta(), ZMetMT_, ZJetM_);
...
void VecbosEESelection::displayEfficiencies() {
...
std::pair<int,int> VecbosEESelection::getBestGoodElePair(std::vector<int> goodElectrons) {
  int theEle1=-1;
  int theEle2=-1;
  float maxPt1=-1000.;
  float maxPt2=-1001.;
...
void VecbosEESelection::setKinematics(int theEle1, int theEle2) {
  // electron candidates
  theEle1_ = theEle1;
  theEle2_ = theEle2;
...
void VecbosEESelection::setKinematics2() {
...
void VecbosEESelection::setElectrons() {
...
// 	{
      ...
      spp_[i] = SigmaiPiP(ele);      
      eop_[i] = eSuperClusterOverPEle[ele];
      fbrem_[i] = fbremEle[ele];
      lh_[i] = likelihoodRatio(ele,*LH);
      e9esc_[i] = E9ESC(ele);
      e25esc_[i] = E25ESC(ele);
      esc_[i] = ESC(ele);
      eseed_[i] = Eseed(ele);
      Utils anaUtils;
      if(_commonSel->getSwitch("PUSubtractionLeptons")) {
        trackerIso_[i] = dr03TkSumPtEle[ele] - rhoFastjet*TMath::Pi()*0.3*0.3;
        hcalIso_[i] = dr03HcalTowerSumEtFullConeEle[ele] - rhoFastjet*TMath::Pi()*0.3*0.3;
        ecalJIso_[i] = dr03EcalRecHitSumEtEle[ele] - rhoFastjet*TMath::Pi()*0.3*0.3;
        ecalGTIso_[i] = scBasedEcalSum03Ele[ele] - rhoFastjet*TMath::Pi()*0.3*0.3;
        if(anaUtils.fiducialFlagECAL(fiducialFlagsEle[ele],isEB))
          combinedIso_[i] = dr03TkSumPtEle[ele] + max(dr03EcalRecHitSumEtEle[ele]-1.0,0.0) + dr03HcalTowerSumEtFullConeEle[ele] - rhoFastjet*TMath::Pi()*0.3*0.3;
        else 
          combinedIso_[i] = dr03TkSumPtEle[ele] + dr03EcalRecHitSumEtEle[ele] + dr03HcalTowerSumEtFullConeEle[ele] - rhoFastjet*TMath::Pi()*0.3*0.3;
      } else {
        trackerIso_[i] = dr03TkSumPtEle[ele];
        hcalIso_[i] = dr03HcalTowerSumEtEle[ele];
        ecalJIso_[i] = dr03EcalRecHitSumEtEle[ele];
        ecalGTIso_[i] = scBasedEcalSum03Ele[ele];
        if(anaUtils.fiducialFlagECAL(fiducialFlagsEle[ele],isEB))
          combinedIso_[i] = dr03TkSumPtEle[ele] + max(dr03EcalRecHitSumEtEle[ele]-1.0,0.0) + dr03HcalTowerSumEtFullConeEle[ele];
        else 
          combinedIso_[i] = dr03TkSumPtEle[ele] + dr03EcalRecHitSumEtEle[ele] + dr03HcalTowerSumEtFullConeEle[ele];
      }
      charge_[i] =  chargeEle[ele];
      int gsf = gsfTrackIndexEle[ele];
      missHits_[i] = expInnerLayersGsfTrack[gsf];
      dist_[i] = convDistEle[ele];
      dcot_[i] = convDcotEle[ele];
      TVector3 p3In(pxGsfTrack[gsf], pyGsfTrack[gsf], pzGsfTrack[gsf]);
      pin_[i] = p3In.Mag();
...
void VecbosEESelection::getBestGoodElePairFunny(std::vector<int> goodElectrons) {
  ...
  for(int iEle=0;iEle<goodElectrons.size();iEle++) {
    ...
    quality.trackerSumPt = dr04TkSumPtEle[eleIndex];
    quality.hcalSumEt = dr04HcalTowerSumEtEle[eleIndex];
    quality.electronIdLH = likelihoodRatio(eleIndex,*LH);
...
// invariant mass
float VecbosEESelection::getMee(int theEle1, int theEle2) {
...
void VecbosEESelection::isEleID(CutBasedEleIDSelector *selector, int eleIndex, bool *eleIdOutput, bool *isolOutput, bool *convRejOutput) {
...
//       dphiin = deltaPhiAtVtxEle[eleIndex] - dphiCorrections(etaEle[eleIndex],phiEle[eleIndex]);
//     }
//   else
//     {
...
//     }
  ...
  selector->SetLikelihood( likelihoodRatio(eleIndex,*LH) );
...
void VecbosEESelection::FillEleIDOptimTree(int eleIndex) {
...
void VecbosEESelection::FillIsolationVertexTree(int closestPV, int iele) {
...
#ifdef USECALOTOWERS
std::vector<CaloTower> VecbosEESelection::getCaloTowers(float zpv, int type) {
...
void VecbosEESelection::getCaloTowersForPFJets(float cptMin, float cptMax, float cchi2, float cetaMax, float cnHits, float cDxy, float cdZ, float cd3d, int iBestPV, int theEle1, int theEle2) 
{
...
std::vector<Jet> VecbosEESelection::buildSortedSISConeCaloJets() { 
...
vector<float> VecbosEESelection::MHTphiJetForW(int theEle1, std::vector<Jet>& cleanedWJets) {
...
float VecbosEESelection::phiJetMETForW(std::vector<Jet>& cleanedWJets, std::vector<TLorentzVector>& tracksForPFJets)
{
  float phiJetMET  = 0.0;
  int njets = (int)cleanedWJets.size();
  if (njets>0) 
...
float VecbosEESelection::MHTphiMETForZ(int theEle1, int theEle2) {
...
void VecbosEESelection::LoadCorrections(){
...
double VecbosEESelection::ParCorr(double thePar, double theMu){
  int iMET_L = 0;
  int iMET_R = 0;
  int iMu_L = 0;
  int iMu_R = 0;
...
double VecbosEESelection::PerpCorr(double thePerp, double theMu){
  int iMET_L = 0;
  int iMET_R = 0;
  int iMu_L = 0;
  int iMu_R = 0;
...
void VecbosEESelection::ConfigCommonSelections(Selection* _selection) {
...
void VecbosEESelection::ConfigZeeSelections(Selection* _selection, Counters *_counter) {
...
void VecbosEESelection::ConfigWenuSelections(Selection* _selection, Counters *_counter) {
...
void VecbosEESelection::displayZeeEfficiencies(Counters _counter){ 
  _counter.Draw();
  if(_commonSel->getSwitch("isData")) {
...
void VecbosEESelection::displayWenuEfficiencies(Counters _counter){ 
  _counter.Draw();
  if(_commonSel->getSwitch("isData")) {
    _counter.Draw("trigger","event");
  } else {
...
void VecbosEESelection::bookBestCandidateHistos() {
...
bool VecbosEESelection::electron_MCmatch_DeltaR(int iMc, int iEle, float deltaR) {
...
float VecbosEESelection::deltaR_MCmatch(int iMc, int iEle) {
...
bool VecbosEESelection::foundZCandidate(int eleIndex, std::vector<int> accEles) {
...
int VecbosEESelection::CountMuons(float etaMax, float ptMin) {
...
float VecbosEESelection::GetDiJetHeaviestMass(std::vector<Jet> cleanedJets) {
...
bool VecbosEESelection::isGoodTrack(int iTrack, float ptMin, float ptMax, float chi2, float etaMax, float nHits) {
...
std::vector<float> VecbosEESelection::jetBTagVariables(Jet jet) {
...
void VecbosEESelection::calcEventBVetoVariables(std::vector<Jet> jets, std::vector<BTagJet> btags) {
...
TLorentzVector VecbosEESelection::getSC4Vector(int eleindex) {
  Utils anaUtils;
  int sclu = superClusterIndexEle[eleindex];
  int pfsclu = PFsuperClusterIndexEle[eleindex];
...
void VecbosEESelection::countBTagJets(std::vector<Jet> jets, std::vector<Jet> btagjets) {
...
int VecbosEESelection::countMatchingB(std::vector<Jet> jets) {
  int nb=0;
